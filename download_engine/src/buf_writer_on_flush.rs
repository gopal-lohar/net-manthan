// this file comes from tokio::io::BufWriter with following changes
// DEFAULT_BUF_SIZE set custom
// additional struct field on_flush
// additional field in with_capacity - on_flush
// on_flush runs in flush_buf
...
pin_project! {
    ...
    pub struct BufWriterWithOnFlush<W> {
        #[pin]
        pub(super) inner: W,
        pub(super) buf: Vec<u8>,
        pub(super) written: usize,
        pub(super) seek_state: SeekState,
        pub(super) on_flush: Box<dyn FnMut(usize) + Send>,
    }
}

impl<W: AsyncWrite> BufWriterWithOnFlush<W> {
    /// Creates a new `BufWriter` with a default buffer capacity. The default is currently 8 KB,
    /// but may change in the future.
    pub fn new(inner: W) -> Self {
        Self::with_capacity(DEFAULT_BUF_SIZE, inner, Box::new(|_| {}))
    }

    /// Creates a new `BufWriter` with the specified buffer capacity.
    pub fn with_capacity(cap: usize, inner: W, on_flush: Box<dyn FnMut(usize) + Send>) -> Self {
        Self {
            inner,
    ...
    fn flush_buf(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        ...
        let bytes_flushed = *me.written;

        if bytes_flushed > 0 {
            me.buf.drain(..bytes_flushed);
        }

        *me.written = 0;

        (*me.on_flush)(bytes_flushed);
...
impl<W: AsyncWrite> AsyncWrite for BufWriterWithOnFlush<W> {
    fn poll_write(
...
impl<W: AsyncWrite + AsyncSeek> AsyncSeek for BufWriterWithOnFlush<W> {
...
impl<W: AsyncWrite + AsyncRead> AsyncRead for BufWriterWithOnFlush<W> {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &mut ReadBuf<'_>,
...
impl<W: AsyncWrite + AsyncBufRead> AsyncBufRead for BufWriterWithOnFlush<W> {
...
impl<W: fmt::Debug> fmt::Debug for BufWriterWithOnFlush<W> {
