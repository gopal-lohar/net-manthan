...
async fn download_part(
    ...
    while let Some(chunk) = stream.next().await {
        ...
        info!(
            "size of the fuckin chunck: {}",
            chunk.len().to_string().yellow()
        );
...
pub async fn handle_download(request: DownloadRequest) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if download_info.supports_parts {
        ...
        while {
            ...
            for part in download_task.lock().unwrap().parts.iter() {
                if part.downloaded.load(Ordering::Relaxed) < part.end - part.start {
                    all_done = false;
                    break;
                }
            }
            ...
            for part in download_task.lock().unwrap().parts.iter() {
                ...
                let progress =
                    (part.downloaded.load(Ordering::Relaxed) as f64 / size as f64) * 100.0;
                write!(
                    handle,
                    "\r\tProgress: [{}] [{}{}]\n",
        ...
        info!("Dwonload finished in multithreaded mode");
    } else {
