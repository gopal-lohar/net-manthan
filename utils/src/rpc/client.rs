...
impl NativeRpcClient {
    ...
    pub async fn connect(settings: &NativeRpcSettings, secret: String) -> Result<Self> {
        #[cfg(unix)]
        {
    ...
    pub async fn send_request(&mut self, request: RpcRequest) -> Result<RpcResponse> {
        let request = Message::request(request, self.secret.clone());
        ...
        loop {
                ...
                .context("Failed to decode message frame")?
            {
                ...
                match response.payload {
                    Payload::Response(response) => return Ok(response),
                    Payload::Request(_) => return Err(anyhow::anyhow!("Unexpected message type")),
                };
            }
