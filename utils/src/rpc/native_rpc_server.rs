...
) -> Result<(NativeServerHandle, tokio::task::JoinHandle<()>)> {
    let (shutdown_tx, shutdown_rx) = oneshot::channel();

    #[cfg(unix)]
    {
        Ok((
            NativeServerHandle { shutdown_tx },
...
async fn handle_unix_connection(
    ...
    loop {
        ...
        while let Some(message_data) = codec.decode(&mut buffer)? {
            trace!("Received message on Unix socket");
            let response_raw = handler.handle_call(&message_data.chunk()).await;
            let mut encoded = BytesMut::new();
            codec.encode(response_raw, &mut encoded)?;
            if let Err(e) = stream.write_all(&encoded).await {
                error!("Failed to write response to Unix stream: {}", e);
                break;
            } else {
                trace!("Response written on Unix Socket")
            }
        }
    }
...
async fn handle_windows_connection(
    ...
    loop {
        ...
        // Process complete messages
        while let Some(message_data) = codec.decode(&mut buffer)? {
            trace!("Received message on named pipe");
            let response_raw = handler.handle_call(&message_data.chunk()).await;
            let mut encoded = BytesMut::new();
            codec.encode(response_raw, &mut encoded)?;
            if let Err(e) = pipe.write_all(&encoded).await {
                ...
                trace!("Response written on named pipe")
            }
        }
    }
